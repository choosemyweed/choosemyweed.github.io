...
# Usage: rake post title="A Title" [date="2012-02-09"] [tags=[tag1, tag2]]
desc "Begin a new strain in #{CONFIG['posts']}"
task :strain do
  abort("rake aborted: '#{CONFIG['posts']}' directory not found.") unless FileTest.directory?(CONFIG['posts'])
  ...
  puts "Creating new strain: #{filename}"
  open(filename, 'w') do |post|
    post.puts "---"
    ...
    post.puts "farm-desc: ''"
    post.puts ""
    post.puts "keywords: [\"#{farm.gsub(/-/,' ')}\"]"
    post.puts ""
    post.puts "categories: [wa, \"#{farm.gsub(/ /,'-')}\"]"
    post.puts "tags: []"
    post.puts ""
    post.puts "strain-base: []"
    post.puts ""
    ...
    post.puts "ts: "
    post.puts " - store: smuggler brothers"
    post.puts "   ubi: 603355893"
    post.puts "   trans: "
    post.puts " - store: the joint"
    post.puts "   ubi: 603352618"
    post.puts "   trans: "
    post.puts " - store: cannerax"
    post.puts "   ubi: 603355874"
    post.puts "   trans: "
    post.puts " - store: loving farms"
    post.puts "   ubi: "
    post.puts "   trans: "
    post.puts " - store: 221 inc"
    post.puts "   ubi: "
    post.puts "   trans: "
    post.puts " - store: high society"
    post.puts "   ubi: "
    post.puts "   trans: "
    post.puts "---"
    post.puts "{% include JB/setup %}"
...
task :store do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "farm-desc: "
    post.puts ""
    post.puts "[comment]: [<state>, <farm>, <brand (only if applies), <strain type (sativa, hybrid, indica)>]"
    post.puts "categories: []"
    post.puts ""
    post.puts "[comment]: [<smell>, <look>, <type of high>]"
    post.puts "tags: []"
    post.puts ""
    post.puts "sativa-per: "
    post.puts "indica-per: "

...
task :strain do
  ...
  open(filename, 'w') do |post|
    ...
    post.puts ""
    post.puts "look-rate"
    post.puts "smell-rate: "
    post.puts "taste-rate: "
    post.puts "high-rate: "
    post.puts "last-rate: "
    post.puts ""
    post.puts "wh: ["
    post.puts "     [smuggler brothers, 00, true],"
    post.puts "     [the joint,         00, false],"
    post.puts "     [cannerax,          00, false],"
    post.puts "     [loving farms,      00, false],"
    post.puts "     [221 inc,           00, false]"
    post.puts "    ]"
    post.puts ""
    post.puts "ts: ["
    post.puts "     [comment]: [<lot number>, <purchase date>, <aggr. rating (of 5)>]"
    post.puts "    ]"
    post.puts "---"
    post.puts "{% include JB/setup %}"
  end
